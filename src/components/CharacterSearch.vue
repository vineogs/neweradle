    ...
    function onEnter() {
        if (personagensFiltrados.value.length > 0) {
            selecionar(personagensFiltrados.value[0])
            emit('try')
            return
        }
    ...
    <template>
        <div :class="venceu ? 'hidden' : ''" class="flex flex-col gap-2">
            ...
            <input v-model="busca" @input="onInput" @keydown.enter.prevent="onEnter" :disabled="venceu" type="text"
